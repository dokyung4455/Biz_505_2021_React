 ...
 * 
 * HTML에서 <input value="b_name">이라는 코드는 결국 키보드에서 b_name input box를 클릭하고 
 ...
 * 때문에 react로 form을 만들고 value속성에 변수명을 지정하는 순간 input box는 readOnly가 된다.
 * 
 * 그렇다면 input에 뭔가를 입력하고 싶은데
 * readonly가 되버리면 input을 사용하는 의미가 없어진다.
...
const Write = () => {
	...
	// setBName() 이라는 함수를 사용하겠다 라는 선언
	const [ b_name, setBName ] = useState("이몽룡");
	 ...
	 * setBName() 함수를 사용하여 b_name 변수에 값을 세팅한다.
	 * 
	 * React 내부 rendering되는 알고리즘
	 * setBName() 함수에 문자열이 전달되면
	 * 내부에서는
	 * 1. 전달된 문자열을 임시 변수에 할당
	 * 	const b_name = 전달된 문자열
	 * 2. 실제 b_name 변수가 가리키고 있는 메모리의 주소를
	 * 	b_name(임시변수) 주소로 변경한다.
	 * 3. 결국 원래 b_name 변수는 그대로 유지되고 새로만들어진 b_name 변수가 생성이 된다.
	 * 4. react 원래 b_name 변수와 새로 생성된 b_name 변수를 비교하여 변화가 있으면 rendering을 한다.
	 * 
	 * 
	 */
	const onWriterChangeHandle = (e)=> {
		...
		setBName(b_writer_text);// 이렇게 사용해야 한다.
		};
	/**
	...
	/**
	 * bbs.변수 타입으로 value를 가지고 있는 input box의 change event 핸들러
	 */
	const onChangeHandle = (e)=> {
		...
		// const _bbs_data = { ...bbsVO,b_date : "2030-01-03"};
	 ...
	 * 2. 현재(write.js)에서 선언된 state객체를 bbs라는 이름의 변수에 담아서 bbsView에게 전달하라.
	 */

	
	return (
		<form>
			<p><input name="b_date" defaultValue={bbs.b_date} onChange={onChangeHandle}/></p>
			<p><input name="b_time" defaultValue={bbs.b_time} onChange={onChangeHandle}/></p>
			<p><input name="b_writer" defaultValue={b_name} onChange={onWriterChangeHandle}/></p>
			<p><input name="b_subject" defaultValue={bbs.b_subject}/></p>
			<p><input name="b_content" defaultValue={bbs.b_content}/></p>
			<div>
